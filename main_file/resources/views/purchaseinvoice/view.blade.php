...
<script>
    ...
    $(document).on('change', 'select[name=item]', function() {
        ...
        $.ajax({
            url: "{{route('purchaseinvoice.items')}}",
            type: 'GET',
            headers: {
...
@endpush
@section('breadcrumb')
<h6 class="h2 d-inline-block mb-0">{{__('Purchase Invoice Detail')}}</h6>
<nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
    <ol class="breadcrumb breadcrumb-links">
        <li class="breadcrumb-item"><a href="#"><i class="fas fa-home"></i></a></li>
        <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{__('Dashboard')}}</a></li>
        <li class="breadcrumb-item"><a href="{{route('purchaseinvoice.index')}}">{{__('Purchase Invoice')}}</a></li>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-auto">
                            <a href="#" data-url="{{route('purchaseinvoice.create.receipt',$invoice->id)}}" data-ajax-popup="true" data-title="{{__('Add Item')}}" class="btn btn-outline-primary btn-sm">
                                <span class="btn-inner--icon"><i class="fa fa-plus"></i></span>
                                <span class="btn-inner--text">{{__('Add Receipt')}}</span>
                            </a>
                        </div>
                        <div class="col-auto">
                            <a href="{{route('purchaseinvoice.send',$invoice->id)}}" class="btn btn-outline-primary btn-sm">
                    ...
                    <div class="row mt-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <div class="font-weight-bold">
                                @if($invoice->status == 0)
                                <span class="badge badge-primary">{{ __(\App\PurchaseInvoice::$statues[$invoice->status]) }}</span>
                                @elseif($invoice->status == 1)
                                <span class="badge badge-info">{{ __(\App\PurchaseInvoice::$statues[$invoice->status]) }}</span>
                                @elseif($invoice->status == 2)
                                <span class="badge badge-default">{{ __(\App\PurchaseInvoice::$statues[$invoice->status]) }}</span>
                                @elseif($invoice->status == 3)
                                <span class="badge badge-danger">{{ __(\App\PurchaseInvoice::$statues[$invoice->status]) }}</span>
                                @elseif($invoice->status == 4)
                                <span class="badge badge-warning">{{ __(\App\PurchaseInvoice::$statues[$invoice->status]) }}</span>
                                @elseif($invoice->status == 5)
                                <span class="badge badge-success">{{ __(\App\PurchaseInvoice::$statues[$invoice->status]) }}</span>
                                @endif
                            </div>
                        </div>
                        ...
                        @if(\Auth::user()->type=='company')
                        <div class="col-md-2">
                            <select class="form-control custom-select status_change font-style" name="status" data-url="{{route('purchaseinvoice.status.change',$invoice->id)}}">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    {!! Form::open(['method' => 'DELETE', 'route' => ['purchaseinvoice.item.delete', $invoice->id,$item->id],'id'=>'item-delete-form-'.$item->id]) !!}
                                    {!! Form::close() !!}
                                    @endif
                                </td>
