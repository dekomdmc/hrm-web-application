...
// Listen to client, and append if data received
...
channel.bind('messaging', function(data) {
    ...
    if (data.from_id == messenger.split('_')[1] && data.to_id == auth_id) {
        if (data.to_id == $("[data-user]").attr('data-user')) {
            (async() => {
                const audio = new Audio('public/assets/sound/notif.mp3');
                await audio.play();
            })();
        }
        // remove message hint
...
async function getLastMessage() {
    const lastMessage = await fetch(`${url}/getLastMessage`, { method: 'post' });
}
    ...
    if (user_id != auth_id) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                $dataDom = $(data.contactItem);
                if (!$dataDom.hasClass('m-list-active')) {
